...
const { fetchSynonyms, countWordsOccurrences, countWordsUniqueOccurrences, maskPDF } = require('../utils/textUtils');
...
fileRouter.post('/countWords', async (req, res) => {
  try {
    ...
    const wordsOccurrences = await countWordsOccurrences(filePath, synonyms);
...
fileRouter.post('/countUniqueWords', async (req, res) => {
  try {
    ...
    if (!req.body.fileName || req.body.fileName.length === 0) return res.status(400).json({ isSuccess: false, error: "fileName not provided" });
    const filePath = path.resolve(__dirname, '../../uploads', req.body.fileName);
    const wordsOccurrences = await countWordsUniqueOccurrences(filePath);
    return res.status(200).json({ isSuccess: true, data: wordsOccurrences });
  } catch (err) {
...
fileRouter.route('/maskPdf').post(async (req, res) => {
  try {
    if (!req.body.userID) return res.status(400).json({ isSuccess: false, error: "User ID not provided" });
    if (!req.body.fileName || req.body.fileName.length === 0) return res.status(400).json({ isSuccess: false, error: "fileName not provided" });
    if (!req.body.word || req.body.word.length === 0) return res.status(400).json({ isSuccess: false, error: "word not provided" });
    const pdfBytes = await maskPDF(path.resolve(__dirname, '../../uploads', req.body.fileName), req.body.word);
    res.setHeader('Content-Type', 'application/pdf');
    res.setHeader('Content-Disposition', 'attachment; filename=masked-pdf.pdf');
    return res.status(200).send({ isSuccess: true, data: pdfBytes });
  } catch (err) {
    return res.status(500).json({ isSuccess: false, error: "Failed To get user file status --> " + err.message });
  }
});
